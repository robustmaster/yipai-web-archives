...
3.  **设置密码**:
    编辑 `config.py`，将 `SERVER_PASSWORD` 的默认值 `"changeme"` 修改为你自己的强密码。
    > **注意**: 如果不修改默认密码，服务将拒绝启动。
...
## 配置说明
所有配置均位于 `config.py` 文件中。建议复制 `config_sample.py` 为 `config.py` 后进行修改。
...
| `SITE_NAME` | **网站名称**。显示在浏览器标题栏和页面顶部的网站标题。 | `"Local Archive"` | ❌ 否 |
| `AUTHOR_NAME` | **维护者名称**。显示在页面底部的版权信息或作者栏。 | `"@Me"` | ❌ 否 |
| `AUTHOR_LINK` | **维护者链接**。点击维护者名称时跳转的链接（如 GitHub 主页或个人博客）。 | `.../yipai...` | ❌ 否 |
| `ITEMS_PER_PAGE` | **每页条数**。首页文章列表每页显示的文章数量。 | `10` | ❌ 否 |

### 环境变量支持
本项目也支持通过环境变量覆盖配置（适用于 Docker 部署），变量名与配置项名称一致（如 `os.environ.get("SITE_NAME")`）。
如果你使用 Docker Compose，可以直接在 `docker-compose.yml` 的 `environment` 节中定义这些变量。
